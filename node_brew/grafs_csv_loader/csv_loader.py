...
class GrafsBatchImageGenerator:
    ...
    RETURN_NAMES = ("PROMPT", "FILENAME", "NEGATIVE", "NEXT_INDEX", "TOTAL_ROWS")
    FUNCTION = "process_batch"
    CATEGORY = "Grafs/CSV"
    
    def process_batch(self, csv_path, base_prompt, base_negative, current_index):
        try:
            with open(csv_path, 'r', encoding='utf-8') as file:
                reader = csv.reader(file)
                ...
                if current_index < total_rows:
                    ...
                    # Just use the image name without extension
                    filename = image_name
                    ...
                    return (full_prompt, filename, base_negative, next_index, total_rows)
                
                print("Batch processing complete!")
                return ("All images processed", "complete", base_negative, current_index, total_rows)
                
        except Exception as e:
            print(f"Error in batch processing: {str(e)}")
            return ("Error", "error", base_negative, current_index, 0)


class GrafsCompareNumbers:
    @classmethod
    def INPUT_TYPES(cls):
        return {
            "required": {
                "value1": ("INT", {"default": 0, "min": 0, "max": 1000}),
                "value2": ("INT", {"default": 0, "min": 0, "max": 1000}),
                "comparison": (["less_than", "greater_than", "equal", "less_equal", "greater_equal"], {"default": "less_than"})
            }
        }
    ...
    RETURN_NAMES = ("RESULT",)
    FUNCTION = "compare"
    CATEGORY = "Grafs/Utils"
    
    def compare(self, value1, value2, comparison):
...
NODE_CLASS_MAPPINGS = {
    ...
    "GrafsCompareNumbers": GrafsCompareNumbers
}
...
NODE_DISPLAY_NAME_MAPPINGS = {
    ...
    "GrafsCompareNumbers": "Grafs Compare Numbers"
} 